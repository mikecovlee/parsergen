#!/usr/bin/env cs
#
# Extended Covariant Script Compiler: v1.0.1
# Generate ECS to CSC/CSP
#
...
class main
    ...
    function visit_import_list(nodes)
        ...
        if idx < nodes.size && (typeid nodes[idx] == typeid parsergen.token_type && nodes[idx].data == "as")
            if wildcard
                throw runtime.exception("Can not bind name alias to wildcard symbol.")
            end
            # Visit term "as"
            ++idx; target.print(" as ")
            # Visit id token
    ...
    function visit_package_stmt(nodes)
        var idx = 0
        if is_package
            throw runtime.exception("Multiple definition of package name.")
        end
        is_package = true
        # Visit term "package"
...
if context.cmd_args.size != 2
    system.out.println("ECSC: Wrong arguments.")
    system.exit(0)
end

var parser = new parsergen.generator
var visitor = new main

parser.add_grammar("ecs-lang", ecs_parser.grammar)
parser.from_file(context.cmd_args.at(1))

if parser.ast != null
    var reg = regex.build("^(.*)\\.ecs$")
    var result = reg.match(context.cmd_args.at(1))
    if !result.empty()
